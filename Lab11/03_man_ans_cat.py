...
class Man:
    ...
    def clean_house(self):
        cprint('{} почистил дом'.format(self.name),color='green')
        self.house.mud -= 100
        self.fullness -= 20

    def read_manga(self):
        cprint('{} читал мангу весь день'.format(self.name), color='green')
        self.fullness -= 10
    ...
    def shopping(self):
        ...
        if self.house.food <= 10 and self.money >= 50:
            self.money -= 50
            self.house.food += 50
            cprint('{} сходил в магазин себе за едой'.format(self.name), color='green')
        ...
        if self.cats and self.house.cat_food <= len(self.cats) * 10:
            if cat_food_need <= self.money:
                ...
                cprint('{} сходил в магазин за едой котам'.format(self.name), color='green')
            elif cat_food_need >= self.money:
                self.house.cat_food += self.money
                self.money = 0
                cprint('{} сходил в магазин за едой котам на последние деньги. Придется влезать в долги (╥﹏╥)'.format(self.name), color='green')
    ...
    def pick_a_cat(self, cat):
        ...
        self.fullness -= 5
        cprint('{} подобрал котика {} в дом'.format(self.name, cat.name), color='green')

    def act(self):
            if self.fullness <= 0:
                cprint('{} умер... (˚ ˃̣̣̥⌓˂̣̣̥ )'.format(self.name), color='red')
                return
            dice = randint(1, 6)
            if self.fullness < 20:
                self.eat()
...
class Cat:
    ...
    def act(self):
        dice = randint(1, 6)
        if self.fullness < 0:
            cprint('Котик {} умер T_T'.format(self.name), color='red')
...
man = Man(name='Минхо ˃̵ᴗ˂̵', house=sweet_home)
print(man)
...
# home_cats = [Cat(name='Хани', house=sweet_home),]
...
cprint('૮ ˶ᵔ ᵕ ᵔ˶ ა', color='magenta')
